...
function MediaFile({ file, folders, onFileMove, onFileDelete }) {
  ...
  // ✅ --- UPDATED LOGIC ---
  // If a thumbnail_path exists, use it.
  // Otherwise, use the main file_path (for regular images).
  const imageUrl = file.thumbnail_path || file.file_path;
  ...
  return (
    ...
    >
      {/* This img tag now correctly shows thumbnails for videos
          and regular images for everything else. */}
      <img
        src={imageUrl}
        alt={file.file_name}
...
function UploadMedia() {
  ...
  // --- ✅ NEW Thumbnail Generation Function ---
  ...
  const fetchLibrary = async () => {
    ...
    try {
      ...
      // ✅ Fetch the new thumbnail_path column
  ...
  const handleUpload = async () => {
    ...
    // 1. Generate thumbnail if it's a video
    if (selectedFile.type.includes('video')) {
      try {
        ...
        alert("Error generating thumbnail: " + thumbError.message);
    ...
    try {
      ...
      // 2. Upload the main file (video or image)
      ...
      // 3. If we have a thumbnail, upload it too
      if (thumbnailBlob) {
      ...
      // 4. Save both URLs to the database
      const fileRecord = {
        file_name: selectedFile.name,
        file_path: mainUrlData.publicUrl,
        thumbnail_path: thumbnailPublicUrl, // Save the new URL
      ...
      // ✅ FIXED: Check if the element exists before trying to clear it
      const rootInput = document.getElementById("file-upload-root");
      if (rootInput) rootInput.value = null;

      const folderInput = document.getElementById("file-upload-folder");
  ...
  const handleFileDelete = async (file) => {
    try {
      ...
      // We also need to delete the thumbnail from storage
      ...
      // 2. Delete thumbnail if it exists
      if (file.thumbnail_path) {
        const thumbStoragePath = file.thumbnail_path.split("/media/")[1];
        const { error: thumbStorageError } = await supabase.storage
          .from("media")
          .remove([thumbStoragePath]);
        if (thumbStorageError) {
          // Log error but don't stop the process
          console.error("Error deleting thumbnail:", thumbStorageError.message);
        }
      }
    ...
    return (
      <div className="p-6 bg-white text-gray-900 min-h-screen">
        <button
          ...
          className="mb-4 font-semibold text-white hover:underline" // Note: text-white might be hard to see
        >
