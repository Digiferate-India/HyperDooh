...
const Loader = forwardRef(({ onLoaded }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    triggerExit() {
      ...
      exitTl
        .to([textContainerRef.current, subtextRef.current, svgPathRef.current.parentElement], {
          opacity: 0,
          duration: 0.5,
          ease: 'power2.in',
        })
...
// All your helper components for the home page go here
const PaperjetLogo = () => ( <svg width="100" height="24" viewBox="0 0 100 24" fill="none" xmlns="http://www.w3.org/2000/svg" > <text x="0" y="18" fontFamily="Inter, sans-serif" fontSize="13" fontWeight="bold" fill="white" > HYPERDOOH </text> </svg> );
    ...
    <>
      ...
      >
        <div className="relative isolate bg-black text-white">
          <div className="absolute inset-x-0 top-0 -z-10 transform-gpu overflow-hidden blur-3xl" aria-hidden="true" > <div className="relative left-[calc(50%)] aspect-[1155/678] w-[36.125rem] -translate-x-1/2 rotate-[190deg] bg-gradient-to-tr from-[#EC1091] to-[#0271E3] opacity-90 sm:left-[calc(50%)] sm:w-[72.1875rem]" style={{ clipPath: "polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)", }} /> </div>
          <main className="relative pt-24 sm:pt-32">
            <div className="mt-19 px-4 sm:px-6 lg:px-8">
              <div className="mx-auto mt-8 max-w-7xl text-center">
                <h1 id="hero-title" className="mt-4 text-4xl font-bold tracking-tight text-white sm:text-6xl" > Revolutionize your digital signage with real-time updates based on audience insights and engagement metrics. </h1>
                <div id="hero-button" className="mt-10 flex items-center justify-center gap-x-6" > <a href="#" className="mb-4 mt-5 rounded-md bg-amber-50 px-4 py-3 text-sm font-semibold text-white shadow-sm" > Book a Demo </a> </div>
              </div>
              <div id="hero-image" className="mt-16 flow-root sm:mt-24">
                <div className="-m-2 rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:-m-4 lg:rounded-2xl lg:p-4"> <img src="https://i.postimg.cc/RZ8j1QFh/carfinale.png" alt="App screenshot" className="w-full rounded-md shadow-2xl ring-1 ring-gray-900/10" /> </div>
              </div>
            </div>
          </main>
        </div>
        {/* I've removed the repetitive full code for the sections below to save space, but you should have it in your project */}
        <div id="stats" className="bg-color-white mt-20"> {/* ... Stats JSX ... */} </div>
        <div className="mt-20"> {/* ... Logos JSX ... */} </div>
        <div className="mt-20"> {/* ... Video JSX ... */} </div>
        <div className="mt-20"> {/* ... Features JSX ... */} </div>
        <div className="py-24 sm:py-32"> {/* ... Pricing JSX ... */} </div>
        <div id="community-section" className="py-16 text-center"> {/* ... Community JSX ... */} </div>
        <footer className="bg-[#fbfbfb]"> {/* ... Footer JSX ... */} </footer>
      </div>
    </>
  );
}
