...
function UploadMedia() {
  ...
  // --- handleUpload function (no change) ---
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("folders")
        .insert({ name: folderName, user_id: user.id }); // This passes our RLS policy
      
      if (error) throw error;

      alert("Folder created!");
      fetchLibrary(); // Refresh the folder list
    } catch (error) {
  ...
  return (
    <div>
      ...
      {/* --- Media Library section (no change) --- */}
