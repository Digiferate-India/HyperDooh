...
s        if (foldersError) throw foldersError;
...
or  if (isLoading) return <div className="p-6">Loading...</div>;
...
      </div>
      <div className="mb-8">
  s     <label htmlFor="screen-select" className="text-lg font-semibold">Screen:</label>
        <select
          id="screen-select"
...
E          {screens.map(screen => <option key={screen.id} value={screen.id}>{screen.custom_name}</option>)}
        </select>
      </div>
...
m          <h2 className="text-xl font-bold">{currentFolder.name}</h2>
            <button
...
s          {mediaInCurrentFolder.map(mediaItem => (
              <MediaCard
                key={mediaItem.id}
                mediaItem={mediaItem}
                screenId={selectedScreen}
                initialAssignment={assignments.get(mediaItem.id)}
d              onAssignmentChange={handleAssignmentChange}
              />
            ))}
          </div>
        </div>
...
s        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {mediaWithoutFolders.map(mediaItem => (
              <MediaCard
                key={mediaItem.id}
U              mediaItem={mediaItem}
        '       screenId={selectedScreen}
                initialAssignment={assignments.get(mediaItem.id)}
...
aS       onSave={handleBulkAssignSave}
        />
      )}
    </div>
  );
