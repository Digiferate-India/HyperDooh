...
function DashboardHome() {
    ...
    async function getDashboardData() {
      try {
        ...
        if (screenIds.length > 0) {
          ...
          // ✅ B. Fetch Recent Faces for the Table
          // We join with audience_profiles to ensure we only get faces from YOUR screens
          const { data: facesResult, error: facesError } = await supabase
            ...
            `)
            .in('audience_profiles.screen_id', screenIds) // Filter by your screens
            .order('created_at', { ascending: false })     // Newest first
            .limit(20);                                    // Limit to 20 rows for the dashboard

          if (facesError) throw facesError;
          setRecentFaces(facesResult || []);
        }

        // --- PROCESS CHART DATA (Same as before) ---
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-full overflow-x-hidden">
      ...
      {/* --- ✅ NEW SECTION: Audience Data Table --- */}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Dwell Time (sec)</th>
              </tr>
            </thead>
