...
function CheckCircleIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-green-600 bg-white rounded-full">
...
function DayPickerDropdown({ selectedDays, onChange }) {
  ...
  const toggleDay = (e, dayId) => {
    ...
    const sortedDays = DAYS_OPTIONS.filter(d => newArray.includes(d.id)).map(d => d.id);
    ...
    <div className="relative w-full">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute z-50 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-xl max-h-60 overflow-y-auto left-0">
...
// --- EXPANDED VIEW MODAL (Individual Media) ---
function SingleMediaModal({ media, existingAssignment, screenId, onClose, onSave, onDelete }) {
  // Initialize state from existing assignment or defaults
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
        ...
        {/* Left: Media Preview */}
        <div className="w-full md:w-5/12 bg-gray-100 p-6 flex flex-col items-center justify-center border-r border-gray-200">
           {media.type?.startsWith('video') ? (
              <video src={media.url} className="max-w-full max-h-[50vh] rounded shadow-lg" controls />
           ) : (
              <img src={media.url || media.file_path} alt={media.name} className="max-w-full max-h-[50vh] object-contain rounded shadow-lg" />
           )}
           <div className="mt-4 text-center">
             <h3 className="font-bold text-lg text-gray-800 break-words">{media.name}</h3>
             {existingAssignment ? (
                <span className="inline-block mt-2 px-3 py-1 bg-green-100 text-green-800 text-xs font-bold rounded-full border border-green-200">
                  ✅ Currently Assigned
                </span>
             ) : (
                <span className="inline-block mt-2 px-3 py-1 bg-gray-200 text-gray-600 text-xs font-bold rounded-full">
                  Not Assigned
                </span>
             )}
           </div>
        ...
        <div className="w-full md:w-7/12 p-6 overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Edit Trigger Options</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input 
                ...
                className="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" 
                placeholder="Auto (Full Video / Default Image Time)" 
              />
            </div>
            ...
            <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
              <div className="flex justify-between items-center mb-4 border-b pb-2">
                <h3 className="font-bold text-gray-700">Schedule Rules</h3>
                <button type="button" onClick={handleClearSchedule} className="text-xs text-red-600 hover:underline font-semibold">Clear Schedule</button>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-1">Active Days</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={gender} onChange={(e) => setGender(e.target.value)} className="w-full border p-2.5 rounded-lg bg-white">
              ...
              <div>
                ...
                <select value={ageGroup} onChange={(e) => setAgeGroup(e.target.value)} className="w-full border p-2.5 rounded-lg bg-white">
              ...
              <div className="col-span-2">
                 <label className="block text-sm font-bold text-gray-700 mb-1">Screen Orientation</label>
                 <select value={orientation} onChange={(e) => setOrientation(e.target.value)} className="w-full border p-2.5 rounded-lg bg-white">
            ...
            <div className="flex justify-between items-center mt-8 pt-6 border-t">
              {existingAssignment ? (
                <button type="button" onClick={handleUnassign} className="text-red-600 hover:text-red-800 font-semibold px-4 py-2 border border-red-200 rounded hover:bg-red-50 transition-colors" disabled={isSaving}>
                   Remove Assignment
                </button>
              ) : (
                <div></div> 
              ...
              <div className="flex gap-3">
                <button type="button" onClick={onClose} className="px-5 py-2 text-gray-600 font-medium hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
                <button type="submit" className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-md disabled:opacity-50 transition-colors" disabled={isSaving}>
...
function AssignContent() {
  ...
  // Handle saving a SINGLE media assignment (Used by the Modal)
  const handleSingleSave = async (assignmentData) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      // Add user_id to data
      const dataToSave = { ...assignmentData, user_id: user?.id };

      const { data, error } = await supabase
        .from('screens_media')
        .upsert(dataToSave, { onConflict: 'screen_id, media_id' })
        .select();
  ...
  // Helper: Render the Thumbnail Grid
      ...
      {mediaList.map(mediaItem => {
        ...
        return (
          <div 
            ...
            className={`
              group relative aspect-square rounded-xl overflow-hidden cursor-pointer shadow-sm hover:shadow-xl transition-all duration-300 border-2 bg-gray-50
              ${isAssigned ? 'border-green-500 ring-2 ring-green-100' : 'border-transparent hover:border-blue-400'}
            `}
          >
            {/* Thumbnail Image */}
            <img 
              src={mediaItem.thumbnail_path || mediaItem.url || mediaItem.file_path}
              alt={mediaItem.name}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
              <p className="text-white text-sm font-semibold truncate">{mediaItem.name}</p>
              <p className="text-gray-300 text-xs">Click to edit</p>
            </div>
  ...
  return (
    <div className="p-6 bg-white text-gray-900 min-h-screen font-sans">
      <div className="mb-6 border-b pb-4">
        <h1 className="text-3xl font-bold mb-2 text-gray-800">Assign Content</h1>
        <p className="text-gray-500">Select a screen, then click any media to configure its display triggers.</p>
      </div>

      {/* Screen Selector */}
      <div className="mb-8 flex items-center bg-blue-50 p-4 rounded-lg border border-blue-100">
        <label htmlFor="screen-select" className="text-lg font-bold text-blue-900 mr-4">Target Screen:</label>
        <select 
          id="screen-select" 
          value={selectedScreen} 
          onChange={(e) => setSelectedScreen(e.target.value)} 
          className="flex-1 max-w-md border-gray-300 border p-2 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 outline-none"
        >
        ...
        {isFetchingAssignments && <span className="ml-4 text-sm text-blue-600 animate-pulse font-medium">Syncing assignments...</span>}
      </div>
      ...
      {currentFolder ? (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex items-center justify-between mb-6">
            <button onClick={() => setCurrentFolder(null)} className="flex items-center text-gray-600 hover:text-blue-600 font-semibold transition-colors group">
              <span className="group-hover:-translate-x-1 transition-transform inline-block mr-1">←</span> Back to Library
            </button>
            <div className="flex items-center gap-4">
               <h2 className="text-2xl font-bold text-gray-800">{currentFolder.name}</h2>
               <button 
                 onClick={() => setIsBulkModalOpen(true)} 
                 className="bg-blue-600 text-white px-4 py-2 text-sm rounded-lg shadow hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors font-medium"
            ...
            {folders.map(folder => (
              <div key={folder.id} onClick={() => setCurrentFolder(folder)} className="group cursor-pointer">
                <div className="border border-gray-100 rounded-xl shadow-sm overflow-hidden bg-white group-hover:shadow-lg group-hover:-translate-y-1 transition-all duration-200">
                  <div className="bg-yellow-50 p-6 flex justify-center items-center">
                    <FolderIcon />
                  </div>
                  <div className="p-3 border-t bg-white">
                    <h3 className="font-semibold text-gray-800 truncate text-sm text-center group-hover:text-blue-600 transition-colors">{folder.name}</h3>
                  </div>
                </div>
              </div>
            ))}
