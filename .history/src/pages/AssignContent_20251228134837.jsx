      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
        ...
        <div className="w-full md:w-1/3 bg-gray-100 p-6 flex flex-col items-center justify-center border-r">
           {media.type?.startsWith('video') ? (
              ...
              <img src={media.thumbnail_path || media.url} alt={media.name} className="w-full h-auto rounded shadow-lg mb-4" />
           )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="col-span-1 md:col-span-2 bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div className="flex justify-between items-center mb-3 border-b pb-2">
                   <h3 className="font-semibold text-gray-700">Schedule Rules</h3>
                   <button 
                     type="button"
                     className="text-xs text-red-500 font-bold"
                     onClick={() => {
                       setStartDate(''); setEndDate(''); setDailyStartTime(''); setDailyEndTime(''); setDaysOfWeek('Mon,Tue,Wed,Thu,Fri,Sat,Sun');
                     }}
                   >
                     Clear Schedule
                   </button>
...
// --- Bulk Assign Form (Same as before) ---
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md text-gray-900 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4"><label className="block text-sm font-bold mb-1">Duration</label><input type="number" value={duration} onChange={(e)=>setDuration(e.target.value)} className="w-full border p-2 rounded" placeholder="Auto" /></div>
          <div className="mb-4"><label className="block text-sm font-bold mb-1">Repeat On</label><DayPickerDropdown selectedDays={daysOfWeek} onChange={setDaysOfWeek} /></div>
          {/* ... Metadata inputs ... */}
...
function AssignContent() {
  ...
  const handleSingleSave = async (assignmentData) => {
    try {
      ...
      const payload = { ...assignmentData, user_id: user.id };

      // Upsert: Insert if new, update if exists
      const { data, error } = await supabase
        .from('screens_media')
        .upsert(payload, { onConflict: 'screen_id, media_id' })
        .select();
  ...
  const renderMediaGrid = (mediaList) => (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
      {mediaList.map(mediaItem => {
        ...
        const imageUrl = mediaItem.thumbnail_path || mediaItem.file_path || mediaItem.url; // Handle various DB naming conventions
        return (
          <div 
            key={mediaItem.id} 
            onClick={() => {
          ...
          >
            ...
            <img 
              src={imageUrl} 
              alt={mediaItem.name || mediaItem.file_name}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <p className="text-white text-xs font-semibold truncate">{mediaItem.name || mediaItem.file_name}</p>
            </div>
