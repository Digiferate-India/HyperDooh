...
function PairScreen() {
  ...
  // ✅ --- NEW: handleUnpair function ---
  const handleUnpair = async (screenId) => {
    if (!window.confirm("Are you sure you want to unpair this screen? This will disconnect the current player.")) {
      return;
    }

    try {
      // Generate a new pairing code for security
      ...
      const { error } = await supabase
        .from("screens")
        .update({
          status: 'unpaired', // Reset status
          paired_at: null,   // Clear paired date
          pairing_code: newPairingCode // Assign new code
        })
        .eq("id", screenId);

      if (error) throw error;

      alert(`Screen unpaired successfully! The new pairing code is: ${newPairingCode}`);
      fetchScreens(); // Refresh the list
    } catch (error) {
      alert("Error unpairing screen: " + error.message);
    }
  };
